...
class EventFactory {
  ...
  async getNetworkInfo() {
    try {
      ...
      _logger.logger.debug("Error getting network info:", error);
      return {};
    }
  }
  ...
  async getDeviceInfo() {
    try {
      const [model, manufacturer, deviceName, userAgent, isTablet] = await Promise.all([_reactNativeDeviceInfo.default.getModel(), _reactNativeDeviceInfo.default.getManufacturer(), _reactNativeDeviceInfo.default.getDeviceName(), _reactNativeDeviceInfo.default.getUserAgent(), _reactNativeDeviceInfo.default.isTablet()]);
      return {
  ...
  async generateContext(context) {
    ...
    const storedTrafficSource = (0, _utils.getStoredTrafficSource)();
