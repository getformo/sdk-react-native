...
import { toChecksumAddress, getValidAddress, toSnakeCase, mergeDeepRight, getStoredTrafficSource } from "../../utils";
...
class EventFactory {
  ...
  async getDeviceInfo() {
    try {
      const [model, manufacturer, deviceName, userAgent, isTablet] = await Promise.all([DeviceInfo.getModel(), DeviceInfo.getManufacturer(), DeviceInfo.getDeviceName(), DeviceInfo.getUserAgent(), DeviceInfo.isTablet()]);
      return {
        os_name: Platform.OS,
  ...
  async generateContext(context) {
    ...
    const mergedContext = mergeDeepRight(defaultContext, context || {});
  ...
  async getEnrichedEvent(formoEvent, context) {
    ...
    const processedEvent = mergeDeepRight(formoEvent, commonEventData);
